...
const api = {
  ...
  getIncomeByCategory: async (userId, period) => {
    const response = await fetch(`http://localhost:5000/api/income/category/${period}/${userId}`, {
  ...
  getIncomeByDateThisMonth: async (userId) => {
    const response = await fetch(`http://localhost:5000/api/income/monthly-dates/${userId}`, {
      headers: api.getAuthHeaders()
    });
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
  ...
  getExpensesByCategory: async (userId, period) => {
    const response = await fetch(`http://localhost:5000/api/expenses/category/${period}/${userId}`, {
      headers: api.getAuthHeaders()
    });
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
  ...
  getExpensesByDateThisMonth: async (userId) => {
    const response = await fetch(`http://localhost:5000/api/expenses/monthly-dates/${userId}`, {
      headers: api.getAuthHeaders()
    });
    if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
    return response.json();
