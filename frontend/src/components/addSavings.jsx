...
const AddSavings = () => {
  ...
  const fetchGoals = async () => {
    try {
      const response = await fetch(`http://localhost:5000/api/savings/goals/${userId}`);
      if (response.ok) {
  ...
  const handleAddMoney = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/savings/transactions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          user_id: userId,
  ...
  const handleDeleteGoal = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/savings/goals/${goalToDelete.id}`, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
      });
